 ...
 function is_valid_user($id)
{
	global $db_mdb2;
	if(is_numeric($id)) {
		$response = $db_mdb2->query("SELECT id FROM users WHERE id=".$db_mdb2->quote($id, 'integer'));
		if (isError($response)) { error($response->getMessage()); return false; }
...
{
	global $db_mdb2;
	$response = $db_mdb2->query("SELECT id FROM users WHERE username=".$db_mdb2->quote($user, 'text'));
	if (isError($response)) { error($response->getMessage()); return false; }
	if ($response->numRows() == 0) {
                 return false;
...
function edit_user($id, $user, $fullname, $email, $perm_templ, $description, $active, $password)
{
	...
	if (($id == $_SESSION["userid"] && $perm_edit_own == "1") || ($id != $_SESSION["userid"] && $perm_edit_others == "1" )) {
		...
		if ($usercheck['username'] != $user) {
			...
			$query = "SELECT id FROM users WHERE username = " . $db_mdb2->quote($user, 'integer');
			$response = $db_mdb2->query($query);
			if (isError($response)) { error($response->getMessage()); return false; }
...
function verify_user_is_owner_zoneid($zoneid) {
	...
	if (is_numeric($zoneid)) {
		$response = $db_mdb2->query("SELECT zones.id 
				FROM zones 
...
function add_new_user($details) {
	...
	$query .= " active) VALUES ("
			...
			. $db_mdb2->quote(md5($details['password']), 'text') . ", "
