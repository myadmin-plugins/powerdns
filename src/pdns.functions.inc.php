...
function get_zone_name_from_id($zid)
{
    ...
    if (is_numeric($zid)) {
        ...
        if ($result) {
            ...
            echo '     <div class="error">'._('Error').': Zone does not exist</div>'.PHP_EOL;
        }
    } else {
        echo '     <div class="error">'._('Error').': '._('Invalid argument(s) given to function').' '.__FUNCTION__.' '._('Not a valid domain').': '.$zid.'</div>'.PHP_EOL;
    }
    return false;
}
...
function update_soa_record($domain_id, $content)
{
    ...
    if (isError($response)) {
        echo '     <div class="error">'._('Error').': '.$response->getMessage().'</div>'.PHP_EOL;
        return false;
    }
    ...
    switch ($type) {
      ...
      case 'SOA':
        ...
        if (!is_valid_rr_soa_content($content)) {
            echo '     <div class="error">'._('Error').': '._('Your content field doesnt have a legit value.').'</div>'.PHP_EOL;
            return false;
        }
      break;
      ...
      default:
        echo '     <div class="error">'._('Error').': '._('Unknown record type.').'</div>'.PHP_EOL;
        return false;
    }
...
function is_valid_hostname_fqdn(&$hostname, $wildcard)
{
    ...
    if (strlen($hostname) > 253) {
        echo '     <div class="error">'._('Error').': '._('The hostname is too long.').'</div>'.PHP_EOL;
        return false;
    }
    ...
    foreach ($hostname_labels as $hostname_label) {
        ...
        if (substr($hostname_label, 0, 1) == '-') {
            echo '     <div class="error">'._('Error').': '._('A hostname can not start or end with a dash.').'</div>'.PHP_EOL;
            return false;
        }
        ...
        if ('' === $hostname_label || strlen($hostname_label) > 63) {
            echo '     <div class="error">'._('Error').': '._('Given hostname or one of the labels is too short or too long.').'</div>'.PHP_EOL;
            return false;
        }
    }
    ...
    if ($hostname_labels[$label_count - 1] == 'arpa' && (substr_count($hostname_labels[0], '/') == 1 xor substr_count($hostname_labels[1], '/') == 1)) {
        ...
        if (count($array) != 2) {
            echo '     <div class="error">'._('Error').': '._('Invalid hostname.').'</div>'.PHP_EOL;
            return false;
        }
        ...
        if (substr_count($hostname, '/') > 0) {
            echo '     <div class="error">'._('Error').': '._('Given hostname has too many slashes.').'</div>'.PHP_EOL;
            return false;
        }
    }

    if ($dns_strict_tld_check && !in_array(strtolower($hostname_labels[$label_count - 1]), $valid_tlds)) {
        echo '     <div class="error">'._('Error').': '._('You are using an invalid top level domain.').'</div>'.PHP_EOL;
        return false;
    }
...
function is_valid_ipv4($ipv4, $answer = true)
{
    if (filter_var($ipv4, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) === false) {
        if ($answer) {
            echo '     <div class="error">'._('Error').': '._('This is not a valid IPv4 address.').'</div>'.PHP_EOL;
        }
        return false;
    }
...
function is_valid_ipv6($ipv6, $answer = true)
{
    if (filter_var($ipv6, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6) === false) {
        if ($answer) {
            echo '     <div class="error">'._('Error').': '._('This is not a valid IPv6 address.').'</div>'.PHP_EOL;
        }
        return false;
    }
...
function is_valid_printable($string)
{
    if (!preg_match('/^[[:print:]]+$/', trim($string))) {
        echo '     <div class="error">'._('Error').': '._('Invalid characters have been used in this record.').'</div>'.PHP_EOL;
        return false;
    }
    return true;
}
...
function is_valid_rr_cname_name($name)
{
    ...
    if (!empty($response)) {
        echo '     <div class="error">'._('Error').': '._('This is not a valid CNAME.').' '._('Did you assign an MX or NS record to the record?').'</div>'.PHP_EOL;
        return false;
    }
...
function is_valid_rr_cname_exists($name, $rid)
{
    ...
    $response = $db_mdb2->queryOne($query);
    if ($response) {
        echo '     <div class="error">'._('Error').': '._('This is not a valid record.').' '._('There is already exists a CNAME with this name.').'</div>'.PHP_EOL;
        return false;
    }
    return true;
}
...
function is_valid_rr_cname_unique($name, $rid)
{
    ...
    $response = $db_mdb2->queryOne($query);
    if ($response) {
        echo '     <div class="error">'._('Error').': '._('This is not a valid CNAME.').' '._('There is already exists an A, AAAA or CNAME with this name.').'</div>'.PHP_EOL;
        return false;
    }
    return true;
}
...
{
    if ($name == $zone) {
        echo '     <div class="error">'._('Error').': '._('Empty CNAME records are not allowed.').'</div>'.PHP_EOL;
        return false;
    }
    return true;
}
...
function is_valid_rr_hinfo_content($content)
{
    ...
    for ($i = 0; $i < 2; $i++) {
        if (!preg_match("/^([^\s]{1,1000})|\"([^\"]{1,998}\")$/i", $fields[$i])) {
            echo '     <div class="error">'._('Error').': '._('Invalid value for content field of HINFO record.').'</div>'.PHP_EOL;
            return false;
        }
    }
...
function is_valid_rr_srv_name(&$name)
{
    ...
    if (!preg_match('/^_[\w-]+$/i', $fields[0])) {
        echo '     <div class="error">'._('Error').': '._('Invalid service value in name field of SRV record.').' (Record: '.$name.')</div>'.PHP_EOL;
        return false;
    }
    if (!preg_match('/^_[\w]+$/i', $fields[1])) {
        echo '     <div class="error">'._('Error').': '._('Invalid protocol value in name field of SRV record.').' (Record: '.$name.')</div>'.PHP_EOL;
        return false;
    }
    if (!is_valid_hostname_fqdn($fields[2], 0)) {
        echo '     <div class="error">'._('Error').': '._('Invalid FQDN value in name field of SRV record.').' (Record: '.$name.')</div>'.PHP_EOL;
        return false;
    }
    $name = implode('.', $fields);
    return true;
...
function is_valid_rr_srv_content(&$content)
{
    ...
    if (!is_numeric($fields[0]) || $fields[0] < 0 || $fields[0] > 65535) {
        echo '     <div class="error">'._('Error').': '._('Invalid value for the priority field of the SRV record.').' (Record: '.$content.')</div>'.PHP_EOL;
        return false;
    }
    if (!is_numeric($fields[1]) || $fields[1] < 0 || $fields[1] > 65535) {
        echo '     <div class="error">'._('Error').': '._('Invalid value for the weight field of the SRV record.').' (Record: '.$content.')</div>'.PHP_EOL;
        return false;
    }
    if ($fields[2] == '' || ($fields[2] != '.' && !is_valid_hostname_fqdn($fields[2], 0))) {
        echo '     <div class="error">'._('Error').': '._('Invalid SRV target.').' (Record: '.$content.')</div>'.PHP_EOL;
        return false;
    }
