...
function get_zones($perm, $userid = 0, $letterstart = 'all', $rowstart = 0, $rowamount = 999999, $sortby = 'name') {
    ...
    if ($perm != "own" && $perm != "all") {
        ...
        if ($letterstart != 'all' && $letterstart != 1) {
            $sql_add .=" AND substring(domains.name,1,1) = " . $db_mdb2->quote($letterstart, 'text') . " ";
...
function get_records_from_domain_id($id, $rowstart = 0, $rowamount = 999999, $sortby = 'name') {
    ...
    if (is_numeric($id)) {
        if ((isset($_SESSION[$id . "_ispartial"])) && ($_SESSION[$id . "_ispartial"] == 1)) {
            $db_mdb2->setLimit($rowamount, $rowstart);
            $result = $db_mdb2->query("SELECT records.id, domains.id, records.name, records.type, records.content, records.ttl, records.prio, records.change_date
            ...
            $result = $db_mdb2->query("SELECT id, domain_id, name, type, content, ttl, prio, change_date
                                    FROM records 
...
function search_zone_and_record($parameters, $permission_view, $sort_zones_by, $sort_records_by) {
    ...
    if ($parameters['reverse']) {
        ...
        $reverse_search_string = $db_mdb2->quote('%' . $reverse_search_string . '%', 'text');
    }
    ...
    if ($parameters['zones']) {
        $zonesQuery = '
            ...
            WHERE
                (domains.name LIKE ' . $db_mdb2->quote($search_string, 'text') .
            ($parameters['reverse'] ? ' OR domains.name LIKE ' . $reverse_search_string : '') . ') ' .
            ($permission_view == 'own' ? ' AND z.owner = ' . $db_mdb2->quote($_SESSION['userid'], 'integer') : '') .
            ' ORDER BY ' . $sort_zones_by;

        $zonesResponse = $db_mdb2->query($zonesQuery);
        if (isError($zonesResponse)) {
    ...
    if ($parameters['records']) {
        $recordsQuery = '
            ...
            LEFT JOIN users u on z.owner = u.id
            WHERE
                (records.name LIKE ' . $db_mdb2->quote($search_string, 'text') . ' OR records.content LIKE ' . $db_mdb2->quote($search_string, 'text') .
        ...
        $recordsResponse = $db_mdb2->query($recordsQuery);
        if (isError($recordsResponse)) {
