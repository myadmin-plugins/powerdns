...
function userUsesLDAP() {
    ...
    $rowObj = $db_mdb2->queryRow("SELECT id FROM users WHERE username=" . $db_mdb2->quote($_SESSION["userlogin"], 'text') . " AND use_ldap=1");
    if ($rowObj) {
        return true;
    }
    return false;
...
function LDAPAuthenticate() {
    ...
    if (isset($_SESSION["userlogin"]) && isset($_SESSION["userpwd"])) {
        ...
        $rowObj = $db_mdb2->queryRow("SELECT id, fullname FROM users WHERE username=" . $db_mdb2->quote($_SESSION["userlogin"], 'text') . " AND active=1");
        if (!$rowObj) {
            if (isset($_POST["authenticate"]))
