		...
		if ($records !== false)
			foreach ($records as $idx => $record) {
				if (isset($GLOBALS['tf']->variables->request['record']) && $GLOBALS['tf']->variables->request['record'] == $record['id']) {
					...
					$table->add_field($table->make_submit('Update') . $table->make_link('choice=none.edit_domain&amp;id=' . $domain_id, '<input type=button value=Cancel>'));
					$table->add_row();
				} else {
					...
					if ($record['type'] != 'SOA')
					{
						$table->add_field($table->make_link('choice=none.edit_domain&id=' . $domain_id . '&record=' . $record['id'], 'Edit'). ' '. $table->make_link('choice=none.edit_domain&id=' . $domain_id . '&record=' . $record['id'] . '&delete=1&csrf_token=' . $csrf_token, 'Delete'));
					} else {
						$table->add_field($table->make_link('choice=none.edit_domain&id=' . $domain_id . '&record=' . $record['id'], 'Edit'));
					}
					$table->add_row();
				}
			}
