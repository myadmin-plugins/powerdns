...
function show_users($id = '', $rowstart = 0, $rowamount = 9999999) {
    ...
    if (is_numeric($id)) {
        ...
        $add = " WHERE users.id!=" . $db_mdb2->quote($id, 'integer');
    }
...
function is_valid_user($id) {
    global $db_mdb2;
    if (is_numeric($id)) {
        $response = $db_mdb2->queryOne("SELECT id FROM users WHERE id=" . $db_mdb2->quote($id, 'integer'));
...
function user_exists($user) {
    global $db_mdb2;
    $response = $db_mdb2->queryOne("SELECT id FROM users WHERE username=" . $db_mdb2->quote($user, 'text'));
    return ($response ? true : false);
}
...
function delete_user($uid, $zones) {
    ...
    if (($uid != $_SESSION['userid'] && !verify_permission('user_edit_others')) || ($uid == $_SESSION['userid'] && !verify_permission('user_edit_own'))) {
        ...
        $query = "DELETE FROM zones WHERE owner = " . $db_mdb2->quote($uid, 'integer');
        $response = $db_mdb2->query($query);
        if (isError($response)) {
            error($response->getMessage());
            return false;
        ...
        $query = "DELETE FROM users WHERE id = " . $db_mdb2->quote($uid, 'integer');
        $response = $db_mdb2->query($query);
        if (isError($response)) {
            error($response->getMessage());
            return false;
...
function edit_user($id, $user, $fullname, $email, $perm_templ, $description, $active, $password) {
    ...
    if (($id == $_SESSION["userid"] && $perm_edit_own == "1") || ($id != $_SESSION["userid"] && $perm_edit_others == "1" )) {
        ...
        $query = "SELECT username FROM users WHERE id = " . $db_mdb2->quote($id, 'integer');
        $response = $db_mdb2->query($query);
        if (isError($response)) {
            error($response->getMessage());
            return false;
        ...
        if ($usercheck['username'] != $user) {
            ...
            $query = "SELECT id FROM users WHERE username = " . $db_mdb2->quote($user, 'text');
            $response = $db_mdb2->queryOne($query);
            if ($response) {
        ...
        $query = "UPDATE users SET
				username = " . $db_mdb2->quote($user, 'text') . ",
				fullname = " . $db_mdb2->quote($fullname, 'text') . ",
				email = " . $db_mdb2->quote($email, 'text') . ",";
        if (verify_permission('user_edit_templ_perm')) {
            $query .= "perm_templ = " . $db_mdb2->quote($perm_templ, 'integer') . ",";
        }
        $query .= "description = " . $db_mdb2->quote($description, 'text') . ", 
...
function change_user_pass($details) {
    ...
    $query = "SELECT id, password FROM users WHERE username = " . $db_mdb2->quote($_SESSION["userlogin"], 'text');
    $response = $db_mdb2->query($query);
    if (isError($response)) {
        error($response->getMessage());
        return false;
...
function get_fullname_from_userid($id) {
    global $db_mdb2;
    if (is_numeric($id)) {
        $response = $db_mdb2->query("SELECT fullname FROM users WHERE id=" . $db_mdb2->quote($id, 'integer'));
        if (isError($response)) {
            error($response->getMessage());
            return false;
        }
...
function get_owner_from_id($id) {
    global $db_mdb2;
    if (is_numeric($id)) {
        $response = $db_mdb2->queryRow("SELECT fullname FROM users WHERE id=" . $db_mdb2->quote($id, 'integer'));
...
function verify_user_is_owner_zoneid($zoneid) {
    ...
    if (is_numeric($zoneid)) {
        $response = $db_mdb2->queryOne("SELECT zones.id FROM zones 
				WHERE zones.owner = " . $db_mdb2->quote($userid, 'integer') . "
				AND zones.domain_id = " . $db_mdb2->quote($zoneid, 'integer'));
        return ($response ? "1" : "0");
    }
    error(ERR_INV_ARG);
}
...
function get_user_detail_list($specific) {
    ...
    if (v_num($specific)) {
        $sql_add = "AND users.id = " . $db_mdb2->quote($specific, 'integer');
    } else {
        if (verify_permission('user_view_others')) {
            $sql_add = "";
        } else {
            $sql_add = "AND users.id = " . $db_mdb2->quote($userid, 'integer');
        }
    }
...
function get_permissions_by_template_id($templ_id = 0, $return_name_only = false) {
    ...
    if ($templ_id > 0) {
        $limit = ", perm_templ_items 
			WHERE perm_templ_items.templ_id = " . $db_mdb2->quote($templ_id, 'integer') . "
...
function get_permission_template_details($templ_id) {
    ...
    $query = "SELECT *
			FROM perm_templ
			WHERE perm_templ.id = " . $db_mdb2->quote($templ_id, 'integer');
...
function add_perm_templ($details) {
    ...
    if (isset($details['perm_id'])) {
        foreach ($details['perm_id'] AS $perm_id) {
            $query = "INSERT INTO perm_templ_items (templ_id, perm_id) VALUES (" . $db_mdb2->quote($perm_templ_id, 'integer') . "," . $db_mdb2->quote($perm_id, 'integer') . ")";
            $response = $db_mdb2->query($query);
            if (isError($response)) {
                error($response->getMessage());
                return false;
...
function update_perm_templ_details($details) {
    ...
    $query = "UPDATE perm_templ 
			...
			WHERE id = " . $db_mdb2->quote($details['templ_id'], 'integer');
    $response = $db_mdb2->query($query);
    if (isError($response)) {
        error($response->getMessage());
        return false;
    ...
    if (isset($details['perm_id'])) {
        foreach ($details['perm_id'] AS $perm_id) {
            $query = "INSERT INTO perm_templ_items (templ_id, perm_id) VALUES (" . $db_mdb2->quote($details['templ_id'], 'integer') . "," . $db_mdb2->quote($perm_id, 'integer') . ")";
            $response = $db_mdb2->query($query);
            if (isError($response)) {
                error($response->getMessage());
                return false;
...
function update_user_details($details) {
    ...
    if (($details['uid'] == $_SESSION["userid"] && $perm_edit_own == "1") ||
        ...
        // user, the username should apparantly changed. If so, check if the "new" 
        // username already exists.
        $query = "SELECT username FROM users WHERE id = " . $db_mdb2->quote($details['uid'], 'integer');
        $response = $db_mdb2->query($query);
        if (isError($response)) {
            error($response->getMessage());
            return false;
        ...
        if ($usercheck['username'] != $details['username']) {
            ...
            // we have been given. User wants a change of username. Now, make
            // sure it doesn't already exist.
            $query = "SELECT id FROM users WHERE username = " . $db_mdb2->quote($details['username'], 'text');
            $response = $db_mdb2->queryOne($query);
            if ($response) {
                error(ERR_USER_EXIST);
                return false;
        ...
        $query = "UPDATE users SET
				username = " . $db_mdb2->quote($details['username'], 'text') . ",
				fullname = " . $db_mdb2->quote($details['fullname'], 'text') . ",
				email = " . $db_mdb2->quote($details['email'], 'text') . ",
				description = " . $db_mdb2->quote($details['descr'], 'text') . ", 
				active = " . $db_mdb2->quote($active, 'integer');

        // If the user is alllowed to change the permission template, set it.
        if ($perm_templ_perm_edit == "1") {
            $query .= ", perm_templ = " . $db_mdb2->quote($details['templ_id'], 'integer');
        }

        // If the user is allowed to change the use_ldap flag, set it.
        if ($perm_is_godlike == "1") {
            $query .= ", use_ldap = " . $db_mdb2->quote($use_ldap, 'integer');
        }
...
function add_new_user($details) {
    ...
    $query .= " active) VALUES ("
            . $db_mdb2->quote($details['username'], 'text') . ", "
            . $db_mdb2->quote($password_hash, 'text') . ", "
            . $db_mdb2->quote($details['fullname'], 'text') . ", "
            . $db_mdb2->quote($details['email'], 'text') . ", "
            . $db_mdb2->quote($details['descr'], 'text') . ", ";
    if (verify_permission('user_edit_templ_perm')) {
        $query .= $db_mdb2->quote($details['perm_templ'], 'integer') . ", ";
    }
    $query .= $db_mdb2->quote($active, 'integer')
            . ")";
    $response = $db_mdb2->query($query);
    if (isError($response)) {
        error($response->getMessage());
