...
if (isset($_POST['submit']) && $_POST['submit']) {
    ...
    $query = "SELECT id, password, email FROM users WHERE email = " . $db_mdb2->quote($email, 'text');
    ...
    if (isset($rinfo['email'])) {
        $newpass = mt_rand();
        $query = "UPDATE users SET password = " . $db_mdb2->quote(md5($newpass), 'text') . " WHERE id = " . $db_mdb2->quote($rinfo['id'], 'integer');
        $response = $db_mdb2->query($query);
        if (isError($response)) {
            error($response->getMessage());
            return false;
